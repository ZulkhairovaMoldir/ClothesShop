...
func (r *CartRepository) RemoveItemFromUserCart(productID uint, customerID uint) error {
	return r.DB.Where("product_id = ? AND customer_id = ?", productID, customerID).Delete(&models.Cart{}).Error
}

func (r *CartRepository) RemoveItemFromGuestCart(productID uint, sessionID string) error {
	return r.DB.Where("product_id = ? AND session_id = ?", productID, sessionID).Delete(&models.Cart{}).Error
}
